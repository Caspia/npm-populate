...
docker rm verdaccio
docker run --name verdaccio -d --network=nginx-proxy -p 4873:4873 --env-file .env --restart=always -v /srv/verdaccio:/verdaccio/storage:rw verdaccio:caspia /usr/local/app/bin/verdaccio --config /verdaccio/conf/$CONFIGFILE --listen http://0.0.0.0:4873